...
from nightly.telemetry._common import (
    SEND_TIMEOUT,
    SESSION_ID,
    base_props,
    get_version,
...
# Renamed from Strix's real gateway (strix.gateway.scarf.sh); this "nightly"
# subdomain isn't a registered Scarf gateway, so sends fail closed (caught
# below) instead of reaching Strix's analytics. Point this at your own Scarf
# gateway if you want working telemetry — see TelemetrySettings.enabled.
_SCARF_ENDPOINT = "https://nightly.gateway.scarf.sh"
...
def _send(event: str, properties: dict[str, Any]) -> bool:
    ...
    try:
        ...
        version = str(props.pop("nightly_version", get_version()) or "unknown")
