"""Build SandboxAgents for root + child Nightly runs."""
...
from nightly.tools.agents_graph.tools import (
    agent_finish,
    create_agent,
...
from nightly.tools.finish.tool import finish_scan
from nightly.tools.load_skill.tool import load_skill
from nightly.tools.notes.tools import (
    create_note,
    delete_note,
    get_note,
    list_notes,
...
from nightly.tools.output_store import bound_and_store, bound_text
from nightly.tools.proxy.tools import (
    list_requests,
    list_sitemap,
    repeat_request,
    scope_rules,
...
from nightly.tools.reporting.tool import (
    create_dependency_report,
    create_vulnerability_report,
    get_report,
    list_reports,
)
from nightly.tools.respond.tool import respond_to_user
from nightly.tools.thinking.tool import think
from nightly.tools.todo.tools import (
    create_todo,
    delete_todo,
    list_todos,
    mark_todo_done,
...
from nightly.tools.web_search.tool import web_search
...
def _with_bounded_result(tool: FunctionTool) -> FunctionTool:
    ...
    if getattr(tool, "_nightly_bounded", False):
        return tool
    ...
    tool._nightly_bounded = True  # type: ignore[attr-defined]
    return tool
...
def _with_coerced_arguments(tool: FunctionTool) -> FunctionTool:
    if getattr(tool, "_nightly_coerced", False):
        return tool
    invoke_tool = tool.on_invoke_tool
    ...
    tool.on_invoke_tool = invoke
    tool._nightly_coerced = True  # type: ignore[attr-defined]
    return tool
...
# ``nightly.runtime.backends.register_backend``: register before the first
# ``build_nightly_agent`` call and every agent (root + children) gets them.
