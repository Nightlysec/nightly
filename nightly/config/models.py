...
from nightly.config.tool_call_ids import TurnCallIdRewriter, dedupe_input
from nightly.config.tool_call_limits import TurnToolCallLimiter
...
if TYPE_CHECKING:
    ...
    from nightly.config.settings import LlmSettings, ReasoningEffort, Settings
...
class _NonStreamingModel(Model):
    ...
    Nightly uses only issues streamed requests, so such a gateway fails every
...
class NightlyProvider(MultiProvider):
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply Nightly config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # Nightly uses LiteLLM's success callback to capture provider-reported cost.
...
def _install_openrouter_stream_cost_capture() -> None:
    ...
    lost and Nightly falls back to a cost-map estimate that is missing entirely
    ...
    from nightly.report.state import streamed_openrouter_costs

    class _NightlyOpenRouterStreamingHandler(OpenRouterChatCompletionStreamingHandler):
    ...
    class _NightlyOpenrouterConfig(OpenrouterConfig):
        def get_model_response_iterator(
            self, streaming_response: Any, sync_stream: bool, json_mode: bool | None = False
        ) -> Any:
            return _NightlyOpenRouterStreamingHandler(
    ...
    litellm.OpenrouterConfig = _NightlyOpenrouterConfig  # type: ignore[misc]
...
def _configure_extra_headers(llm: LlmSettings) -> None:
    ...
    regardless of the ``NIGHTLY_LLM`` prefix.
    """
...
def _register_litellm_cost_callback() -> None:
    ...
    from nightly.report.state import litellm_cost_callback
