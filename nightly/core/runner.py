...
from nightly.agents.factory import build_nightly_agent, make_child_factory
...
from nightly.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from nightly.core.execution import (
...
from nightly.core.hooks import BudgetExceededError, ReportUsageHooks, recomputed_budget_flags
...
from nightly.core.sessions import open_agent_session
from nightly.report.state import get_global_report_state
from nightly.runtime import session_manager
from nightly.telemetry.logging import set_scan_id, setup_scan_logging
from nightly.tools.output_store import (
    WORKSPACE_SPILL_DIR,
...
if TYPE_CHECKING:
    ...
    from nightly.runtime.status import StatusSink
...
async def run_nightly_scan(
    *,
    ...
    """Run or resume one Nightly scan against a sandbox.
    ...
    logger.info(
        "%s Nightly scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set NIGHTLY_LLM env or pass model= to run_nightly_scan().",
        )
    ...
    from nightly.tools.notes.tools import hydrate_notes_from_disk
    from nightly.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        run_config = RunConfig(
            ...
            model_provider=NightlyProvider(),
        ...
        root_agent = build_nightly_agent(
            name="Root Agent",
            skills=skills,
            is_root=True,
            scan_mode=scan_mode,
        ...
        logger.warning(
            ...
            "Resume with 'nightly --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Nightly scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("Nightly scan %s done", scan_id)
        teardown_logging()
