...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `nightly/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **Nightly defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `nightly/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `nightly.core.sessions.strip_all_images_from_session`, invoked from
  `nightly/core/execution.py`.
