...
- **Nightly config:** `containers/Dockerfile` sets `AGENT_BROWSER_*` env
  (executable path, UA, launch args, screenshot dir).
- **Skill:** `nightly/skills/tooling/agent_browser.md` — **always-loaded**
  into every agent prompt by `nightly/agents/prompt.py:_resolve_skills`.
