"""NightlyDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
_SANDBOX_NETWORK_ENV = "NIGHTLY_DOCKER_SANDBOX_NETWORK"
...
def _apply_resource_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    mem_limit = os.environ.get("NIGHTLY_SANDBOX_MEM_LIMIT", "").strip()
    if mem_limit:
        create_kwargs["mem_limit"] = mem_limit

    shm_size = os.environ.get("NIGHTLY_SANDBOX_SHM_SIZE", "").strip()
    if shm_size:
        create_kwargs["shm_size"] = shm_size

    cpus = os.environ.get("NIGHTLY_SANDBOX_CPUS", "").strip()
    if cpus:
    ...
    pids_limit = os.environ.get("NIGHTLY_SANDBOX_PIDS_LIMIT", "").strip()
    if pids_limit:
...
def _apply_log_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    ``max-size * max-file``. Set ``NIGHTLY_SANDBOX_LOG_MAX_SIZE`` to ``0``/``off``
    to opt back out to docker's default."""
    max_size = os.environ.get("NIGHTLY_SANDBOX_LOG_MAX_SIZE", "50m").strip()
    if max_size.lower() in ("0", "off", "none", "unlimited"):
        return
    max_file = os.environ.get("NIGHTLY_SANDBOX_LOG_MAX_FILE", "3").strip() or "3"
...
def _apply_run_labels(create_kwargs: dict[str, Any]) -> None:
    run_id = os.getenv("NIGHTLY_RUN_ID")
    if not run_id:
        return
    labels = create_kwargs.setdefault("labels", {})
    if not isinstance(labels, dict):
        return
    labels["nightly-run-id"] = run_id
    run_type = os.getenv("NIGHTLY_RUN_TYPE")
    if run_type:
        labels["nightly-run-type"] = run_type


class NightlyDockerSandboxSession(DockerSandboxSession):
    sandbox_network: str = ""
...
class NightlyDockerSandboxClient(DockerSandboxClient):
    ...
    nightly_bind_mounts: list[dict[str, Any]] | None = None
    ...
    async def _create_container(
        ...
        *,
        ...
        # Nightly delta from the SDK body: drop ``entrypoint`` override and
        ...
        # Nightly injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        # Nightly injection: local source trees, sorted shallowest-first so a
        # nested spec lands on top of the tree it covers.
        bind_mounts = self.nightly_bind_mounts or ()
        if bind_mounts:
    ...
    async def create(self, **kwargs: Any) -> SandboxSession:
        ...
        if network and isinstance(inner, DockerSandboxSession):
            inner.__class__ = NightlyDockerSandboxSession
            cast("NightlyDockerSandboxSession", inner).sandbox_network = network
        return session
