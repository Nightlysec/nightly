"""Sandbox backend registry — selected via NIGHTLY_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`NightlyDockerSandboxClient` to inject NET_ADMIN /
    ...
    Nightly manages session lifetime explicitly via ``client.delete()`` so we
    ...
    client = NightlyDockerSandboxClient(docker.from_env())
    client.nightly_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown NIGHTLY_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
