...
telemetry live here so ``nightly.interface.main`` (the CLI) and
``nightly.interface.tui.runtime`` (interactive setup) depend on one module
...
from nightly.config import Settings, codex, load_settings
...
async def preflight_model_connection(
    ...
    *,
    ...
    from nightly.config.models import NightlyProvider, configure_sdk_model_defaults
    ...
    model = NightlyProvider().get_model(model_name)
...
def _persist_run_record(args: argparse.Namespace) -> None:
    from nightly.report.writer import write_run_record
