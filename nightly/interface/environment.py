...
def validate_environment() -> None:
    ...
    if codex.subscription_model(settings.llm.model):
        if not codex.is_authenticated():
            console.print(
                f"[red]NIGHTLY_LLM={settings.llm.model} uses your ChatGPT subscription, "
                "but you're not signed in.[/] Run [cyan]nightly auth login chatgpt[/] first."
            )
            sys.exit(1)
    ...
    if not settings.llm.model:
        missing_required_vars.append("NIGHTLY_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "NIGHTLY_LLM":
                ...
                error_text.append("NIGHTLY_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "NIGHTLY_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("NIGHTLY_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export NIGHTLY_LLM='openai/gpt-5.4'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_BASE":
                    ...
                    error_text.append(
                        "export NIGHTLY_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )
