...
from nightly.config import load_settings, persist_current
from nightly.core.agents import AgentCoordinator
from nightly.core.hooks import BudgetExceededError
from nightly.core.runner import run_nightly_scan
...
from nightly.interface.tui.backend import TuiBackendServer, TuiController
from nightly.interface.tui.backend.live_view import TuiLiveView
from nightly.interface.tui.sidecar import (
    check_return_code,
    child_environment,
    launch_tui_process,
    package_version,
...
from nightly.report.state import ReportState, set_global_report_state
...
class GoTuiRuntime:
    ...
    async def _run_scan(self) -> None:
        image = str(load_settings().runtime.image or "nightly-sandbox:latest")
        try:
    ...
    def binary_command() -> list[str]:
        ...
        if (source / "go.mod").is_file() and shutil.which("go"):
            return ["go", "run", "./cmd/nightly-tui"]
        packaged = get_nightly_resource_path("bin", tui_executable())
        if packaged.is_file():
            return [str(packaged)]
        raise RuntimeError(
            "Bubble Tea TUI binary not found. Reinstall Nightly from an official platform wheel."
        )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            env["NIGHTLY_VERSION"] = package_version()
