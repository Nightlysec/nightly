"""UI-independent state and command controller for interactive Nightly clients."""
...
from nightly.config.models import is_recommended_or_frontier_model
from nightly.config.settings import DEFAULT_MAX_TURNS
from nightly.interface.tui.backend.live_view import TuiLiveView
from nightly.interface.tui.backend.projection import (
    MAX_TERMINAL_EVENTS,
...
from nightly.interface.utils import is_subscription_run


if TYPE_CHECKING:
    import argparse

    from nightly.report.state import ReportState
...
class TuiController:
    ...
    async def _start(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        model = (load_settings().llm.model or "").strip()
        if not model:
            raise ValueError("No model configured. Set NIGHTLY_LLM first.")
    ...
    async def _open_viewer(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from nightly.interface.tui.backend.messages import (
                send_user_message_to_agent,
            )
            from nightly.interface.viewer.server import (
                authorized_url,
                bundle_is_built,
                serve,
            )
