...
def tui_executable() -> str:
    return "nightly-tui.exe" if os.name == "nt" else "nightly-tui"
...
async def _launch_posix_tui_process(
    ...
    try:
        env["NIGHTLY_TUI_FD"] = str(child_socket.fileno())
...
async def _launch_windows_tui_process(
    ...
    try:
        ...
        env.update({"NIGHTLY_TUI_ADDR": f"{host}:{port}", "NIGHTLY_TUI_TOKEN": token})
