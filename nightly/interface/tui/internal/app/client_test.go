...
func TestConnectFromEnvironmentAuthenticatesTCPTransport(t *testing.T) {
	...
	t.Setenv("NIGHTLY_TUI_ADDR", listener.Addr().String())
	t.Setenv("NIGHTLY_TUI_TOKEN", "one-use-token")
	t.Setenv("NIGHTLY_TUI_FD", "")
	...
	if os.Getenv("NIGHTLY_TUI_ADDR") != "" || os.Getenv("NIGHTLY_TUI_TOKEN") != "" {
...
func TestConnectFromEnvironmentRequiresCompleteTransport(t *testing.T) {
	t.Setenv("NIGHTLY_TUI_FD", "")
	t.Setenv("NIGHTLY_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("NIGHTLY_TUI_TOKEN", "")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "NIGHTLY_TUI_ADDR and NIGHTLY_TUI_TOKEN") {
		t.Fatalf("error = %v, want missing transport error", err)
	}
}

func TestConnectFromEnvironmentPrefersInheritedDescriptor(t *testing.T) {
	t.Setenv("NIGHTLY_TUI_FD", "not-a-number")
	t.Setenv("NIGHTLY_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("NIGHTLY_TUI_TOKEN", "token")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "invalid NIGHTLY_TUI_FD") {
