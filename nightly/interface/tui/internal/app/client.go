...
func ConnectInherited(fdValue string) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid NIGHTLY_TUI_FD: %w", err)
	}
	file := os.NewFile(uintptr(fd), "nightly-tui-ipc")
	if file == nil {
		return nil, fmt.Errorf("invalid NIGHTLY_TUI_FD %d", fd)
	}
...
func ConnectFromEnvironment() (*Client, error) {
	if fd := os.Getenv("NIGHTLY_TUI_FD"); fd != "" {
		_ = os.Unsetenv("NIGHTLY_TUI_FD")
		return ConnectInherited(fd)
	}

	address := os.Getenv("NIGHTLY_TUI_ADDR")
	token := os.Getenv("NIGHTLY_TUI_TOKEN")
	_ = os.Unsetenv("NIGHTLY_TUI_ADDR")
	_ = os.Unsetenv("NIGHTLY_TUI_TOKEN")
	if address == "" || token == "" {
		return nil, fmt.Errorf("NIGHTLY_TUI_FD or NIGHTLY_TUI_ADDR and NIGHTLY_TUI_TOKEN are required")
	}
