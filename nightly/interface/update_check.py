"""Update notifications and self-update for the nightly CLI.
...
result in ``~/.nightly``, a non-intrusive notice with the upgrade command
for the detected install method, and a ``nightly --update`` self-update
...
from nightly.telemetry._common import get_version
...
_CACHE_PATH = Path.home() / ".nightly" / "update-check.json"

_background_thread: threading.Thread | None = None


def _is_disabled() -> bool:
    return bool(os.environ.get("NIGHTLY_NO_UPDATE_CHECK")) or any(
        os.environ.get(key)
...
def get_upgrade_command(method: str | None = None) -> str:
    ...
    commands = {
        "binary": "nightly --update",
        "pipx": "pipx upgrade nightly-agent",
        "uv": "uv tool upgrade nightly-agent",
        "pip": "pip install --upgrade nightly-agent",
    }
...
def notify_update(console: Console) -> None:
    ...
    if not latest:
        return
    console.print(
        f"[#eab308]A new version of nightly is available:[/] "
...
def run_package_upgrade(console: Console, method: str) -> bool:
    ...
    if result.returncode != 0:
        ...
        return False
    console.print("[#22c55e]✓ nightly updated — restart the scan to use the new version[/]")
    return True
...
def prompt_update_if_available(console: Console) -> bool:
    ...
    Returns True if nightly was updated (caller should re-exec / exit).
    """
    latest = get_available_update()
    ...
    choice = Prompt.ask("Update nightly?", choices=["y", "n", "s"], default="n")
    console.print()
    if choice == "s":
        skip_version(latest)
        return False
...
def _download_and_replace(version: str, target: str, console: Console) -> bool:
    ...
    filename = f"nightly-{version}-{target}{archive_ext}"
    url = f"https://github.com/{GITHUB_REPO}/releases/download/v{version}/{filename}"
    binary_name = f"nightly-{version}-{target}" + (".exe" if is_windows else "")
...
def self_update(console: Console | None = None, version: str | None = None) -> bool:
    ...
    if not is_binary_install():
        method = get_install_method()
        console.print(
            f"[#eab308]This nightly was installed via {method};[/] "
    ...
    if not latest:
        console.print("[bold red]Could not determine the latest nightly version.[/]")
        return False

    current = get_version()
    if current != "unknown" and not _is_newer(latest, current):
        console.print(f"[#22c55e]nightly {current} is already the latest version.[/]")
        return True
    ...
    _write_cache(latest_version=latest, checked_at=time.time())
    console.print(f"[#22c55e]✓ Updated nightly to {latest}[/]")
    return True
