"""`nightly auth` — ChatGPT subscription sign-in (login / status / logout).

Signing in only stores OAuth tokens (``~/.nightly/subscription-auth.json``); model
selection stays with ``NIGHTLY_LLM``. A ``chatgpt/<model>`` NIGHTLY_LLM runs on the
subscription.
"""
...
from nightly.config import codex, load_settings
...
_USAGE = "Usage:\n  nightly auth login chatgpt [--manual]\n  nightly auth status\n  nightly auth logout"


def run_auth(argv: list[str]) -> int:
    """Entry point for ``nightly auth …``. Returns a process exit code."""
    console = Console()
    # Bare `nightly auth` (no subcommand) defaults to login.
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="nightly auth login", add_help=True)
    parser.add_argument(
        "provider",
        nargs="?",
...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print("[yellow]Not signed in.[/] Run [cyan]nightly auth login chatgpt[/] to sign in.")
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        console.print(f"  Runs use the subscription (NIGHTLY_LLM=[bold]{settings.llm.model}[/]).")
    else:
        console.print(
            "  [yellow]Note:[/] set [cyan]NIGHTLY_LLM[/] to e.g. [cyan]chatgpt/gpt-5.4[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("NIGHTLY_LLM", style="bold white")
    ...
    text.append("nightly --target https://example.com", style="bold cyan")
    console.print()
    console.print(
        Panel(
            text,
...
def _logo_img_tag() -> str:
    """Return an ``<img>`` for the Nightly logo as an inline data URI, or "".
...
<body>
  <a class="topbar" href="https://strix.ai" target="_blank" rel="noopener"
     aria-label="Nightly — strix.ai">
    <!--LOGO-->
    <span>Nightly</span>
  </a>
  <div class="brand">Nightly</div>
  <h1>You're signed in</h1>
  <main class="card">
    <div class="badge">✓</div>
    <p class="msg">Nightly is connected to your ChatGPT subscription. Head back to your
