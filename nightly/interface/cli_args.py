"""Command-line argument parsing for the ``nightly`` scan entrypoint."""
...
from nightly.config import apply_config_override
from nightly.config.settings import DEFAULT_MAX_TURNS
from nightly.core.paths import run_dir_for, runtime_state_dir
from nightly.interface.scan_setup import attach_workspace_mount, build_targets_info
from nightly.interface.update_check import self_update
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Nightly Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  nightly --target https://example.com

  # GitHub repository analysis
  nightly --target https://github.com/user/repo
  nightly --target git@github.com:user/repo.git

  # Local code analysis
  nightly --target ./my-project

  # API spec test (OpenAPI/Swagger file or Postman collection export)
  nightly --target ./openapi.yaml --target https://api.example.com
  nightly --target ./collection.postman_collection.json

  # Postman collection pulled live by id (needs POSTMAN_API_KEY); optional environment
  nightly --target postman://<collection-uuid> --target https://api.example.com
  nightly --target "postman://<collection-uuid>?env=<environment-uuid>"

  # Domain penetration test
  nightly --target example.com

  # IP address penetration test
  nightly --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  nightly --target https://github.com/user/repo --target https://example.com
  nightly --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  nightly --target-list ./targets.txt

  # Custom instructions (inline)
  nightly --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  nightly --target example.com --instruction-file ./instructions.txt
  nightly --target https://app.com --instruction-file /path/to/detailed_instructions.md
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"nightly {get_version()}",
    )

    parser.add_argument(
        "--update",
        action="store_true",
        help="Update nightly to the latest version and exit. Self-updates the "
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.nightly/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./nightly_runs/). "
...
def _load_resume_state(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    from nightly.report.writer import read_run_record
