...
// TUI strips them in nightly/interface/tui/renderers/shell_renderer.py). Only a
...
function cleanOutput(raw: string, command: string = ""): string {
  ...
  if (cleaned.trim()) {
    ...
    for (const line of lines) {
      ...
      // Skip [NIGHTLY_N]$ prompt lines
      if (/^\[NIGHTLY_\d+\]\$\s*/.test(line)) continue;
    ...
    // Strip trailing [NIGHTLY_N]$ lines
    while (filtered.length > 0 && /^\[NIGHTLY_\d+\]\$\s*/.test(filtered[filtered.length - 1])) {
      filtered.pop();
    }
