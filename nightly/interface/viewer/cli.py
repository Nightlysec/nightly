"""`nightly view [<run>]` command: serve a run's viewer UI locally."""
...
from nightly.core.paths import (
    RUNS_DIR_NAME,
    latest_run_dir,
    run_dir_for,
    run_record_path,
...
from nightly.interface.viewer.server import authorized_url, bundle_is_built, serve
from nightly.interface.viewer.transcript import read_run_summary
...
def run_view(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="nightly view",
        description="Open a local web view of a Nightly run (live or finished).",
    )
    parser.add_argument(
        "run",
        nargs="?",
    ...
    if not bundle_is_built():
        console.print(
            ...
            "Build it with: [cyan]cd nightly/interface/viewer/frontend && npm ci && npm run build[/]"
        )
        raise SystemExit(1)
