...
from nightly.core.paths import run_record_path
from nightly.interface.viewer import auth
...
# port is appended (``nightly_viewer_session_<port>``) because browsers scope
...
SESSION_COOKIE_PREFIX = "nightly_viewer_session"
...
class _ViewerState:
    def __init__(
        ...
        # The nightly_runs directory that holds the launched run; used to
        ...
        # standalone ``nightly view`` / finished runs, so steering is unavailable.
...
def _make_handler(state: _ViewerState) -> type[BaseHTTPRequestHandler]:
    class ViewerHandler(BaseHTTPRequestHandler):
        server_version = "NightlyViewer/1.0"
        ...
        def _handle_report_send(self) -> None:
            ...
            from nightly.interface.viewer.report_pdf import build_encrypted_report
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``nightly view``
    ...
    ``None`` (standalone ``nightly view``), steering is reported unavailable.
    """
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="nightly-viewer", daemon=True)
    thread.start()
