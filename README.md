<p align="center">
  <a href="https://strix.ai/">
    <img src="https://github.com/usestrix/.github/raw/main/imgs/cover.png" alt="Nightly Banner" width="100%">
  </a>
</p>
...
> [!TIP]
> **New!** Nightly integrates seamlessly with GitHub Actions and CI/CD pipelines. Automatically scan for vulnerabilities on every pull request and block insecure code before it reaches production - [Get started with no setup required](https://app.strix.ai).
...
<div align="center">
  <a href="https://strix.ai">
    <img src=".github/screenshot.png" alt="Nightly Demo" width="1000" style="border-radius: 16px;">
  </a>
</div>
...
> [!NOTE]
> First run automatically pulls the sandbox Docker image. Results are saved to `nightly_runs/<run-name>`

---

## ☁️ Nightly Platform

Try the Nightly full-stack penetration testing platform at **[app.strix.ai](https://app.strix.ai)** - sign up for free, connect your repos and domains, and launch a pentest in minutes.
...
## 🤖 Use Nightly from Your Coding Agent
...
Nightly agents come equipped with a comprehensive offensive security toolkit - the same tools used by professional penetration testers and ethical hackers:
...
Nightly identifies, validates, and exploits a wide range of security vulnerabilities across the OWASP Top 10 and beyond:
...
Point Nightly at an API contract and it tests every declared endpoint instead of
...
nightly --target ./openapi.yaml --target https://api.your-app.com

# Postman collection export
nightly --target ./collection.postman_collection.json --target https://api.your-app.com

# Postman collection pulled live by id (no manual export)
export POSTMAN_API_KEY="PMAK-..."
nightly --target postman://<collection-uuid>

# ...with a Postman environment to resolve {{baseUrl}} / token variables
nightly --target "postman://<collection-uuid>?env=<environment-uuid>"
```
...
nightly --target https://your-app.com --instruction "Perform authenticated testing using credentials: user:pass"

# Multi-target testing (source code + deployed app)
nightly -t https://github.com/org/app -t https://your-app.com

# Targets from a file, one target per non-empty, non-comment line
nightly --target-list ./targets.txt

# White-box source-aware scan (local repository)
nightly --target ./app-directory --scan-mode standard

# Focused testing with custom instructions
nightly --target api.your-app.com --instruction "Focus on business logic flaws and IDOR vulnerabilities"

# Provide detailed instructions through file (e.g., rules of engagement, scope, exclusions)
nightly --target api.your-app.com --instruction-file ./instruction.md

# Force PR diff-scope against a specific base branch
nightly -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main
```

### Headless Mode

Run Nightly programmatically without interactive UI using the `-n/--non-interactive` flag - perfect for servers and automated jobs. The CLI prints real-time vulnerability findings and the final report before exiting. Exits with non-zero code when vulnerabilities are found.

```bash
nightly -n --target https://your-app.com
```

### CI/CD (GitHub Actions)

Nightly can be added to your pipeline to run a security test on pull requests with a lightweight GitHub Actions workflow:

```yaml
name: nightly-penetration-test
...
> [!TIP]
> In CI pull request runs, Nightly automatically scopes quick reviews to changed files.
...
export NIGHTLY_REASONING_EFFORT="high"  # control thinking effort (default: high, quick scan: medium)
```

> [!NOTE]
> Nightly automatically saves your configuration to `~/.nightly/cli-config.json`, so you don't have to re-enter it on every run.

#### Sign in with a ChatGPT subscription

Instead of a metered API key, you can run Nightly on your ChatGPT Plus/Pro subscription:

```bash
nightly auth login chatgpt      # sign in with your ChatGPT account

export NIGHTLY_LLM="chatgpt/gpt-5.4"   # chatgpt/<model> runs on the subscription
nightly --target ./app-directory

nightly auth status             # show the active sign-in
nightly auth logout             # forget the sign-in
```
...
**Love Nightly?** Give us a ⭐ on GitHub!

## Acknowledgements

Nightly builds on the incredible work of open-source projects like [LiteLLM](https://github.com/BerriAI/litellm), [Caido](https://github.com/caido/caido), [Nuclei](https://github.com/projectdiscovery/nuclei), [Playwright](https://github.com/microsoft/playwright), and [Bubble Tea](https://github.com/charmbracelet/bubbletea). Huge thanks to their maintainers!


> [!WARNING]
> **Authorized use only.** Nightly actively tests the targets you point it at, so only run it against systems you own or have **explicit, written permission** to test, and stay within the agreed scope. Unauthorized testing is illegal in most jurisdictions.
> You alone are responsible for obtaining authorization and complying with the law. Nightly is provided "as is" with no warranty or liability for misuse.
