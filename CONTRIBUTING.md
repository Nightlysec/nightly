...
Thank you for your interest in contributing to Nightly! This guide will help you get started with development and contributions.
...
4. **Run Nightly in development mode**
   ```bash
   uv run nightly --target https://example.com
   ```

## 📚 Contributing Skills

Skills are specialized knowledge packages that enhance agent capabilities. See [nightly/skills/README.md](nightly/skills/README.md) for detailed guidelines.
...
`nightly view` serves a prebuilt web UI whose source lives in
`nightly/interface/viewer/frontend/` (a Vite + React project) and whose built output is
committed to `nightly/interface/viewer/static/` and shipped in the package. End users never
run a JS build. If you change anything under `nightly/interface/viewer/frontend/`, rebuild
and commit the output:

```bash
make viewer   # or: cd nightly/interface/viewer/frontend && npm ci && npm run build
```

Commit both the source change and the regenerated `nightly/interface/viewer/static/`.
...
`nightly/bin/nightly-tui`, and assigns the current platform tag. It requires Go
1.24.x or newer and fails rather than producing a wheel without the sidecar.
`scripts/build.sh` and `nightly.spec` are likewise strict for frozen PyInstaller
releases.
