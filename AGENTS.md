...
Nightly is an open-source autonomous AI pentesting tool. This file is for AI coding agents that want to **use** Nightly (run security scans) or **contribute** to it.

## Using Nightly from an agent
...
- `managed-pentesting-with-nightly` — drive the managed app.strix.ai platform via REST (no local Docker/LLM needed)
- `fix-security-vulnerabilities-with-nightly` — remediate findings and re-run Nightly to verify
- `ci-security-scanning-with-nightly` — add PR scanning to CI/CD (self-hosted CLI or managed app)
...
- **Open-source CLI (self-hosted):** free, fully local, BYO LLM key, needs Docker. Best for local dev loops, air-gapped/offline, and full control.
  ```bash
  ./scripts/install.sh                                # install (run from a clone of this repo; no hosted release yet)
  export NIGHTLY_LLM="openai/gpt-5.4"                 # any LiteLLM model id
  export LLM_API_KEY="<key>"
  nightly -n -t ./ --scan-mode quick --max-budget 10  # headless scan; always use -n
  ```
  ...
  - Artifacts in `nightly_runs/<run-name>/`: `penetration_test_report.md`, `vulnerabilities/*.md`, `vulnerabilities.json`, `findings.sarif` (SARIF 2.1.0), `run.json`.
...
- **Managed cloud (app.strix.ai):** no Docker, no LLM key, no local install; adds team dashboards, scheduling, PR reviews, and downloadable PDF/DOCX reports (Enterprise plan). Best in sandboxed/CI environments and for teams. Use it when local infra isn't available.
  ...
  curl -sS https://app.strix.ai/api/v1/scans -H "Authorization: Bearer $NIGHTLY_API_TOKEN" \
...
- Tests: `uv run pytest`.
- Run from source: `uv run nightly --target <target>`.
- Layout: `nightly/agents` (agent graph + prompts), `nightly/tools` (proxy, browser, terminal, scanners), `nightly/runtime` (Docker sandbox), `nightly/report` (findings, SARIF), `nightly/skills` (internal knowledge packs the pentest agents load — different from the consumer skills in `skills/`), `nightly/interface` (CLI/TUI), `containers/` (sandbox image).
