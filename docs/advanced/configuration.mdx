---
title: "Configuration"
description: "Environment variables for Nightly"
---

Configure Nightly using environment variables or a config file.

## LLM Configuration

<ParamField path="NIGHTLY_LLM" type="string" required>
...
<ParamField path="NIGHTLY_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="NIGHTLY_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="NIGHTLY_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="NIGHTLY_DEDUPE_MODEL" type="string">
  Model used to judge whether a candidate finding duplicates an existing report.
  Falls back to `NIGHTLY_LLM` when unset.
</ParamField>
...
<ParamField path="NIGHTLY_DEDUPE_REASONING_EFFORT" type="string">
...
<ParamField path="NIGHTLY_TELEMETRY" default="0" type="string">
  Telemetry toggle. Off by default in this fork — set to `1`, `true`, `yes`, or `on` to enable telemetry (Supabase, Scarf).
</ParamField>

<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, Nightly keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, Nightly still writes complete run telemetry locally to:

```bash
nightly_runs/<run_name>/events.jsonl
```

When remote vars are set, Nightly dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="NIGHTLY_IMAGE" default="ghcr.io/usenightly/nightly-sandbox:1.3.0" type="string">
...
<ParamField path="NIGHTLY_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

## Sandbox Configuration

<ParamField path="NIGHTLY_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="NIGHTLY_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

Nightly stores configuration in `~/.nightly/cli-config.json`. You can also specify a custom config file:

```bash
nightly --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "NIGHTLY_LLM": "openai/gpt-5.4",
    "LLM_API_KEY": "sk-...",
    "NIGHTLY_REASONING_EFFORT": "high"
  }
}
```
...
export NIGHTLY_SANDBOX_EXECUTION_TIMEOUT="300"
