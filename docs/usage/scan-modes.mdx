...
Nightly offers three scan modes to balance speed and thoroughness.

## Quick

```bash
nightly --target ./app --scan-mode quick
```
...
```bash
nightly --target ./app --scan-mode standard
```
...
```bash
nightly --target ./app --scan-mode deep
```
