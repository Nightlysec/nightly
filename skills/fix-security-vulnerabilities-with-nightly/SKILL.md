---
name: fix-security-vulnerabilities-with-nightly
...
# Fix Nightly findings and verify

Turn validated Nightly findings into minimal, correct fixes — and prove they work by re-scanning.

## 1. Triage

Get the findings from wherever the scan ran:

- **OSS CLI** — artifacts in `nightly_runs/<run-name>/`:
...
- **Cloud (app.strix.ai)** — fetch the scan's `vulnerabilities[]` via `GET /api/v1/scans/{scanId}` (or `GET /api/v1/vulnerabilities` org-wide). Each carries `severity, cwe, endpoint, method, impact, technical_analysis, poc_description, poc_script_code` and, for code findings, `code_file`/`code_diff`/`code_before`/`code_after`. See the **managed-pentesting-with-nightly** skill for auth.

Order work by severity: critical → high → medium → low. Every Nightly finding was validated with a working proof-of-concept, so do not dismiss findings as false positives without re-testing the PoC yourself.
...
## 3. Verify by re-running Nightly
