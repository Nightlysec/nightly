---
name: penetration-testing-with-nightly
...
# Run a Nightly pentest

Nightly runs autonomous AI pentesting agents that dynamically exploit a target and only report findings validated with a working proof-of-concept. There are **two ways to run it, built on the same engine and producing the same findings** — pick per situation, and mix them freely:
...
3. **LLM configured** — two environment variables:
   ```bash
   export NIGHTLY_LLM="openai/gpt-5.4"      # any LiteLLM model id (openai/..., anthropic/..., openrouter/...)
...
nightly -n -t ./ --scan-mode standard --max-budget 10

# Deployed app / API (black-box)
nightly -n -t https://staging.example.com --max-budget 20

# Repo + deployed app together (best coverage)
nightly -n -t https://github.com/org/app -t https://staging.example.com

# Focused testing with credentials or scope hints
nightly -n -t https://app.example.com \
...
| `--resume RUN_NAME` | Resume a prior run from `nightly_runs/`. |
...
A `0` is not proof of full coverage: if `--max-budget`/`--max-turns` is reached before the scan completes, it wraps up early and still exits `0`. When you need assurance the scan finished, give it enough budget and check `nightly_runs/<run>/run.json`: a hard budget stop leaves `status: "stopped"`, but an agent that wrapped up early on a budget *warning* still calls `finish_scan` and records `"completed"` — so also sanity-check the run's cost against `--max-budget` and the report's stated coverage before treating a clean result as full coverage.

### Reading results

Artifacts land in `nightly_runs/<run-name>/`:
...
Full details, asset registration, polling, reports, PR reviews, schedules, and webhooks are in the **managed-pentesting-with-nightly** skill. Minimal launch-and-poll:

```bash
export NIGHTLY_API_TOKEN="<token>"   # org-scoped bearer, from Settings → API Access at app.strix.ai
BASE=https://app.strix.ai/api/v1

# 1. Launch a scan against an already-registered domain/repo asset
scan_id=$(curl -sS "$BASE/scans" \
  -H "Authorization: Bearer $NIGHTLY_API_TOKEN" -H "Content-Type: application/json" \
...
curl -sS "$BASE/scans/$scan_id" -H "Authorization: Bearer $NIGHTLY_API_TOKEN" | jq '.status'

# 3. Read validated findings from the scan detail's `vulnerabilities[]`, or export SARIF
curl -sS "$BASE/scans/$scan_id/sarif" -H "Authorization: Bearer $NIGHTLY_API_TOKEN" -o findings.sarif
```
...
Summarize findings by severity (critical/high/medium/low/info) and include the PoC evidence. To remediate and verify fixes (via either path), use the **fix-security-vulnerabilities-with-nightly** skill. To wire scanning into CI/CD, use the **ci-security-scanning-with-nightly** skill.
